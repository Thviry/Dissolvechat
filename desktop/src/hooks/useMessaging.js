  ...
  const handleIncoming = useCallback(async (env) => {
    ...
    if (env.payload) {
      ...
      if (inner.t === "Message") {
        setMessages((prev) => [
          ...prev,
          { dir: "in", peerId: inner.from, text: inner.text, ts: inner.ts, msgId: inner.msgId || randomId() },
        ]);
      }
    ...
    if (env.body?.msg?.cipher) {
      ...
      if (env.t === "Message") {
        setMessages((prev) => [
          ...prev,
          { dir: "in", peerId: env.from, text: plaintext, ts: env.ts, msgId: msg.msgId || randomId() },
        ]);
      }
    }
    ...
    const publishAndStart = async () => {
      try {
        ...
        if (discoverable && handle?.trim()) {
          ...
          await fetch(
            `${import.meta.env.VITE_API_URL || "http://localhost:3001"}/directory/publish`,
            {
              method: "POST",
  ...
  const sendMsg = useCallback(async (peerId, text) => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (resp.ok) {
        setMessages((prev) => [
          ...prev,
          { dir: "out", peerId, text: text.trim(), ts, msgId },
        ]);
        return;
      }
